...
const generateRouletteImage = (num: number): string => {
  const bgColor = num === 0 ? "#22c55e" : isEven(num) ? "#000000" : "#ef4444";
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><rect width="100" height="100" fill="${bgColor}"/><text x="50" y="65" font-family="Arial" font-size="40" fill="white" text-anchor="middle" font-weight="bold">${num}</text></svg>`;
  return `data:image/svg+xml;base64,${btoa(svg)}`;
};

const PRIZES = ROULETTE_NUMBERS.map((num) => ({ id: `number-${num}`, image: generateRouletteImage(num), text: num.toString() }));

const generatePrizeList = () => {
  const list = [];
  for (let i = 0; i < 8; i++) {
    for (const prize of PRIZES) {
      list.push({ ...prize, id: `${prize.id}-rep-${i}`, originalNumber: parseInt(prize.text) });
    }
  }
  return list;
};
...
export function Roulette({ currentPoints, onPointsUpdate }: RouletteProps) {
  ...
  const handleBetTypeSelect = (type: BetType) => { setBetType(type); setBetChoice(null); };
  const handleBetChoiceSelect = (choice: number) => setBetChoice(choice);
  ...
  const handleSpin = async () => {
    ...
    try {
      const result = await api.games.rouletteSpin(betAmount, betType, betChoice);
      ...
      const repetitionIndex = 4;
      const targetIndex = (repetitionIndex * PRIZES.length) + result.winningNumber;
  ...
  const handleNextRound = () => {
    setPhase("betting");
    setBetType(null); setBetChoice(null); setBetAmount(0);
    setWinningNumber(null); winningNumberRef.current = null;
    setResultMessage(""); setError(null); setIsWin(false);
    setStartSpinning(false); setPrizeIndex(0);
  };
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        {phase === "betting" && (
          <>
            <Card className="bg-muted/50">
              <CardContent className="pt-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Button variant={betType === "odd-even" ? "default" : "outline"} onClick={() => handleBetTypeSelect("odd-even")} className="h-auto py-4 flex flex-col">
                    <span className="text-lg font-bold">Impair/Pair</span>
                    <Badge variant="secondary" className="mt-2">x2</Badge>
                  </Button>
                  <Button variant={betType === "number" ? "default" : "outline"} onClick={() => handleBetTypeSelect("number")} className="h-auto py-4 flex flex-col">
                    <span className="text-lg font-bold">Numero (0-35)</span>
            ...
            {betType === "odd-even" && (
              <Card className="bg-muted/50">
                <CardContent className="pt-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Button variant={betChoice === 1 ? "default" : "outline"} onClick={() => handleBetChoiceSelect(1)} className="h-auto py-4">
                      <span className="text-lg font-bold">🔴 Impair</span>
                    </Button>
                    <Button variant={betChoice === 2 ? "default" : "outline"} onClick={() => handleBetChoiceSelect(2)} className="h-auto py-4">
            ...
            {betType === "number" && (
              <Card className="bg-muted/50">
                <CardContent className="pt-6">
                  <Label className="text-base font-semibold block mb-4">Choisissez un numero (0-35):</Label>
                  <div className="grid grid-cols-6 gap-2 max-h-60 overflow-y-auto p-2">
                    {ROULETTE_NUMBERS.map((num) => (
                      <Button key={num} variant={betChoice === num ? "default" : "outline"} onClick={() => handleBetChoiceSelect(num)} className={`h-10 w-10 p-0 text-sm font-bold ${betChoice === num ? "" : getNumberColor(num)}`}>
                        {num}
                      </Button>
                    ))}
                  </div>
            ...
            {betChoice !== null && (
              <div className="space-y-4">
                ...
                <div className="flex justify-between gap-2">
                  ...
                  <Button variant="outline" onClick={() => handleBetAmountChange(betAmount + 50)} className="flex-1">+50</Button>
                  <Button variant="outline" onClick={() => handleBetAmountChange(betAmount + 100)} className="flex-1">+100</Button>
                  <Button variant="outline" onClick={() => handleBetAmountChange(betAmount + 1000)} className="flex-1">+1000</Button>
                  <Button variant="secondary" onClick={() => handleBetAmountChange(currentPoints)} className="flex-1">All-In</Button>
                </div>
              </div>
            )}
        ...
        {phase === "spinning" && (
          <div className="text-center py-8 space-y-6">
            ...
            <div className="flex justify-center items-center min-h-[300px]">
              <RoulettePro prizes={PRIZE_LIST} prizeIndex={prizeIndex} start={startSpinning} onPrizeDefined={handlePrizeDefined} spinningTime={5} options={{ stopInCenter: true, withoutAnimation: false }} />
            </div>
          </div>
        )}
        ...
        {phase === "result" && winningNumber !== null && (
          <div className="text-center py-8 space-y-6">
            <div className={`text-6xl w-20 h-20 mx-auto rounded-full flex items-center justify-center font-bold text-3xl ${getNumberColor(winningNumber)}`}>{winningNumber}</div>
            <p className={`text-2xl font-bold ${isWin ? "text-green-600" : "text-destructive"}`}>{resultMessage}</p>
            <Card className="bg-muted/50 text-left">
              <CardHeader><CardTitle className="text-base">Details du pari</CardTitle></CardHeader>
